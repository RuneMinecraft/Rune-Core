...
@Cmd(names="warn", perms="punish.warn")
public class WarnCommand extends ICommand {
    @Override
    public void execute(CommandSender sender, String[] args) {
        if (args.length == 0) {
            Message.create(sender(), Messages.ARGUMENTS+" &cUsage: /warn <player> <reason> <modifiers>").send(false);
            return;
        }
        ...
        Punishment punishment = Punishment.of(PunishmentType.WARN, user.uuid(),
                isPlayer() ? player().getUniqueId() : UUID.fromString("CONSOLE"),
                reason, 1000*60*60*24*7,  modifiers.toArray(new Class[0]));

        punishment.punish();
        if (target != null) {
            Message.create(target, "&4You have been warned by "+punishment.staff()+".", "&4Expires in: "+ DateUtils.fromLong(punishment.punishmentLength()), "&4Reason: "+punishment.reason()).send(false);
        }
    }
}
